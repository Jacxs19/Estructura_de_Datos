...
public class TDAArbol<E> implements Tree<E>{
    ...
    public void eliminarUltimaHijo(Position<E> p){
        TNodo<E> nodo = checkPosition(p);
        if(raiz==nodo)
            throw new InvalidPositionException("No se puede eliminar la raiz");
        PositionList<TNodo<E>> hijos_padre = nodo.getPadre().getHijos();
        if(hijos_padre.last().element()==nodo){
            Position<TNodo<E>> eliminar = hijos_padre.last();
            if(!nodo.getHijos().isEmpty()){
                PositionList<TNodo<E>> hijos_eliminado = nodo.getHijos();
                for(TNodo<E> insertarHijos : hijos_eliminado){
                    hijos_padre.addLast(insertarHijos);
                    insertarHijos.setPadre(nodo.getPadre());
                }
            }
            hijos_padre.remove(eliminar);
            nodo.setElemento(null);
            nodo.setPadre(null);
            size--;
        }
