package ar.edu.uns.cs.ed.TpN7.ej5;
...
public class ArbolBinarioEnlazado<E> implements BinaryTree<E>{
    ...
    public void eliminarSubarbol(Position<E> p){
        BTNode<E> nodo = checkPosition(p);      //C1
        if(nodo==raiz){                         //C2
            eliminar(p);                //O(n)      //Peor caso
            raiz=null;                          //C2.2
            size=0;
            return;                             //C2.3
        }
        BTNode<E> padre = checkPosition(nodo.getPadre());           //C3
        if(padre.getLeft()==p ){              //Pertenece al arbol          //C4
            padre.setLeft(null);                                       //C5
            eliminar(p);                                    //Cicla segun la cantidad de nodos parientes (hijos, nietos, etc.) tenga.
            return;                                                 //C6
        }
        if(padre.getRight()==p){                                    //C7
            padre.setRight(null);                             //C8
            eliminar(p);                                    //Cicla segun la cantidad de nodos parientes (hijos, nietos, etc.) tenga.   
            return;                                                 //C9
        }
        throw new InvalidPositionException("La posicion no pertenece al arbol");        //C10
    }

    private void eliminar(Position<E> p){               //Recorrido en postOrden
        BTNode<E> nodo = checkPosition(p);                      //C1
        if(nodo.getLeft()!=null)                                //C2
            eliminar(nodo.getLeft());                   //Cicla segun la cantidad de nodos parientes (hijos, nietos, etc.) tenga el sub-arbol.
        if(nodo.getRight()!=null)                               //C3
            eliminar(nodo.getRight());                  //Cicla segun la cantidad de nodos parientes (hijos, nietos, etc.) tenga el sub-arbol.
        nodo.setElemento(null);                               //C4
        nodo.setPadre(null);                                 //C5
        nodo.setLeft(null);
        nodo.setRight(null);
        size--;
    }
    //T_eliminarSubArbol(n)= En el peor de los casos, la posicion pasada es la raiz, por lo que el recorrido va a ser el size
    //del arbol (n), ya que visito a todos los nodos. Por lo tanto este metodo es de O(n)
