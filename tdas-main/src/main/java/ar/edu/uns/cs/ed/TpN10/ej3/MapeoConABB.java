package ar.edu.uns.cs.ed.TpN10.ej3;
...
public class MapeoConABB<K,V> implements Map<K,V> {
    ...
    public boolean sucesorInorder(K key, K key2){
        if(key == null || key2 ==null)
            throw new InvalidKeyException("Llave invalida");
        NodoMapABB<K,V> nodo1 = busqueda(key, raiz);            //O(h), donde h es la altura del arbol
        if(nodo1.getHijoDer()==null)
            return false;
        NodoMapABB<K,V> minimo = buscarMinimo(nodo1.getHijoDer());  //O(h), donde h es la altura del sub-arbol derecho
        return minimo.getKey().equals(key2);
    }
    //T_sucesorInorden(n)= O(h), donde h es la altura del arbol, en el peor caso en donde 
    //el ABB esta desbalanceado seria O(n), donde n es la cantidad de nodos del arbol.
